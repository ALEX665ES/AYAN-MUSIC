...
photo_url = "https://telegra.ph/file/6ccdb4eb31fbaf6f45885.jpg"
...
async def sudoers_list(client, message: Message):
    keyboard = [
        [InlineKeyboardButton("𝖵𝗂𝖾𝗐 𝖲𝗎𝖽𝗈𝗅𝗂𝗌𝗍", callback_data="check_sudo_list")]
    ]
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    "𝖵𝗂𝖾𝗐 𝖮𝗐𝗇𝖾𝗋", url=f"tg://openmessage?user_id={OWNER_ID}"
                )
            ]
        )
        ...
        for user_id in SUDOERS:
            if user_id != OWNER_ID:
                try:
                    ...
                    user_mention = (
                        user.mention if user else f"**✨ 𝖲𝗎𝖽𝗈 {count} 𝖨𝖣:** {user_id}"
                    )
                    caption += f"**✨ 𝖲𝗎𝖽𝗈 ** {count} **»** {user_mention}\n"
                    button_text = f"𝖵𝗂𝖾𝗐 𝖲𝗎𝖽𝗈 {count} ๏ "
                    keyboard.append(
                        [
                            InlineKeyboardButton(
        ...
        keyboard.append(
            [InlineKeyboardButton("𝖡𝖺𝖼𝗄", callback_data="back_to_main_menu")]
        )
